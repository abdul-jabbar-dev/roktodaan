...
function useLocationSelect(step5Address: { division: string; district: string; upazila: string }) {
  ...
  useEffect(() => {
    if (division.length && step5Address.division) {
      const match = division.find(d => d.name === step5Address.division || d.bn_name === step5Address.division);
  ...
  useEffect(() => {
    if (district.length && step5Address.district) {
      const match = district.find(d => d.name === step5Address.district || d.bn_name === step5Address.district);
  ...
  useEffect(() => {
    if (upazila.length && step5Address.upazila) {
      const match = upazila.find(u => u.name === step5Address.upazila || u.bn_name === step5Address.upazila);
