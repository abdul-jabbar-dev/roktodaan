...
export const DonorRow: React.FC<{ donor: DonorInfo }> = ({ donor }) => (
    <tr className="border-b even:bg-red-50/20 border-gray-200 last:border-b-0 hover:bg-gray-100 transition">
...
}> = ({ currentPage, totalPages, onPageChange, SET_ITEMS_PER_PAGE }) => {
        ...
        <nav className="flex flex-wrap justify-center items-center gap-2 mt-6">
            ...
            <div className="relative">
                <select
                    ...
                    className="appearance-none bg-gray-100 border border-gray-200 rounded-lg py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-red-500"
                >
...
const DonorTable: React.FC<DonorTableProps> = ({ userLocation, searchRadius, mockDonors }) => {
    ...
    const filteredDonors = useMemo(() => {
        if (!userLocation || !searchRadius || searchRadius === 0) return mockDonors;

        return mockDonors.filter((donor: DonorInfo) => {
            const coords = donor?.address?.coords;
            if (typeof coords !== 'object' || !coords) return false;
                ...
                <table className="min-w-full bg-white">
                    ...
                    <tbody>
                        {paginatedDonors.length > 0 ? (
                            paginatedDonors.map((donor) => <DonorRow key={donor.id} donor={donor} />)
                        ) : (
                            <tr>
